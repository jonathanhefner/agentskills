...
- Strip YAML frontmatter before returning (see [What the model receives](#what-the-model-receives) below)
...
**Full file**: The model sees the entire `SKILL.md` including YAML frontmatter. This is the default with file-read activation and also a valid choice for dedicated tools. No transformation needed — the model naturally focuses on the body content and ignores the frontmatter.

**Body only (frontmatter stripped)**: The harness parses and removes the YAML frontmatter, returning only the markdown instructions. Slightly cleaner — less noise for the model — but requires a parsing step at activation time.

Both approaches work in practice. Stripping frontmatter is cleaner; returning the full file is simpler to implement.
