...
In this tutorial, you'll create a skill that teaches an agent to roll dice using true randomness — something LLMs fundamentally cannot do on their own.
...
To roll a die, generate a random number from 1 to the number of sides using the following command:
